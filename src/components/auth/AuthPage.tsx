...
const AuthPage: React.FC = () => {
  ...
  return <div className="min-h-screen flex flex-col lg:flex-row items-center justify-center gap-8 p-4 lg:p-8 bg-background">
      {/* Branding Image Section */}
      <div className="hidden lg:block">
        <img src={main4Branding} alt="Main4 - CMMS Software Developed by Engineers for Engineers" className="max-h-[500px] w-auto object-contain rounded-lg" />
      </div>
      ...
      <div className="flex items-center justify-center bg-background p-4 py-[30px] px-px">
        <div className="w-full max-w-md border-2 border-solid py-0">
          ...
          {view === 'login' && <LoginForm onToggleMode={handleToggleMode} />}
          {view === 'register' && <RegisterForm onToggleMode={handleToggleMode} onRegistrationComplete={handleRegistrationComplete} />}
          {view === 'verification-pending' && <EmailVerificationPending email={pendingEmail} onBackToLogin={handleBackToLogin} />}
        </div>
      </div>
    </div>;
};
