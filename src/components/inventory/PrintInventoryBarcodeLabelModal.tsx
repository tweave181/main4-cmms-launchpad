...
import { generateInventoryBarcodeLabelPDF, downloadPDF } from '@/utils/inventoryBarcodeLabelUtils';
import { supabase } from '@/integrations/supabase/client';
import { format } from 'date-fns';

interface PrintInventoryBarcodeLabelModalProps {
  isOpen: boolean;
  onClose: () => void;
  sku: string;
  partName: string;
...
const LABEL_CONFIG = {
  barcodeWidth: 1.8,
  barcodeHeight: 50,
  printBarcodeWidth: 45,
  printBarcodeHeight: 12,
  width: 50,
  height: 25,
  fontSize: { sku: '10pt', name: '7pt' },
};

export const PrintInventoryBarcodeLabelModal: React.FC<PrintInventoryBarcodeLabelModalProps> = ({
  isOpen,
  onClose,
  sku,
  partName,
  ...
  const handleDownloadAction = async () => {
    ...
    try {
      ...
      const doc = await generateInventoryBarcodeLabelPDF(
        { sku, partName },
        barcodeDataUrl,
        includePartName,
        copies
      );
      downloadPDF(doc, `${sku.replace(/\//g, '-')}-label.pdf`);
  ...
  const handleBrowserPrintAction = async () => {
    ...
    const labelsHtml = Array(copies).fill(null).map(() => `
      <div class="label">
        <div class="text-content">
          ...
          ${includePartName && partName ? `<span class="part-name">${partName}</span>` : ''}
        </div>
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Inventory Label - ${sku}</title>
          <style>
            @page { 
            ...
            * {
            ...
            .sku { 
              ...
              color: #000; 
              font-size: ${LABEL_CONFIG.fontSize.sku}; 
            }
            .part-name { 
              color: #000; 
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-center">
              <div className="flex flex-col items-center gap-1 p-3 bg-white rounded-lg border shadow-sm">
                ...
                <div ref={barcodeRef} id="inventory-barcode-container">
                  <Barcode
                    value={sku}
                    format="CODE128"
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label htmlFor="include-part-name" className="text-sm">Include Part Name</Label>
                <Switch
                  id="include-part-name"
                  checked={includePartName}
                  onCheckedChange={setIncludePartName}
                />
              </div>
      ...
      <AlertDialog open={showReprintConfirm} onOpenChange={setShowReprintConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              A bar code label was already printed for this part on{' '}
