...
export const SupplierSelector: React.FC<SupplierSelectorProps> = ({
  ...
  const filteredSuppliers = addresses?.filter(address => {
    ...
    const companyName = address.company_details?.company_name || '';
    const contactName = address.contact_name || '';
                  ...
                  <Building2 className="h-4 w-4 text-muted-foreground" />
                   <div className="min-w-0 flex-1">
                     <div className="font-medium truncate">
                       {supplier.company_details?.company_name || supplier.contact_name || 'Unnamed Supplier'}
                     </div>
                     {supplier.company_details?.company_name && supplier.contact_name && (
