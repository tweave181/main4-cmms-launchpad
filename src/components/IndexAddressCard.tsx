...
export const IndexAddressCard: React.FC<IndexAddressCardProps> = ({
  ...
  const getTypeBadges = () => {
    ...
    if (address.is_contact) badges.push({ label: 'Contact', variant: 'secondary' as const });
    if (address.is_supplier) badges.push({ label: 'Supplier', variant: 'outline' as const });
    if (address.is_manufacturer) badges.push({ label: 'Manufacturer', variant: 'outline' as const });
    if (address.is_contractor) badges.push({ label: 'Contractor', variant: 'outline' as const });
    if (address.is_other) badges.push({ label: 'Other', variant: 'outline' as const });
  ...
  return (
    <Card 
      className={cn(
        "group cursor-pointer transition-all duration-200 hover:shadow-lg hover:scale-[1.02] border-border",
