  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('assets').select('category').not('category', 'is', null).eq('status', 'active');
      if (error) throw error;
        ...
        <div className="flex flex-col space-y-4 flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-auto">
            {isLoading ? <div className="flex items-center justify-center h-32">
                <p>Loading assets...</p>
              </div> : assets.length === 0 ? <div className="flex items-center justify-center h-32">
