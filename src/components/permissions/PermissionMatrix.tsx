  ...
  return <Card>
      ...
      <CardContent>
        <div className="space-y-6">
          {Object.entries(groupedPermissions).map(([resource, permissions]) => <div key={resource} className="space-y-3">
              <div className="flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {permissions.map(perm => <div key={perm.id} className="flex items-center space-x-2 p-3 rounded-lg border bg-card hover:bg-accent/50 transition-colors">
                    <Checkbox id={perm.id} checked={selectedPermissions.has(perm.id)} onCheckedChange={() => handlePermissionToggle(perm.id)} disabled={rolePermsLoading} />
