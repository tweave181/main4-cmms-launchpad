...
const REQUIREMENTS_TXT_CONTENT = `fastapi>=0.109.0
...
const README_MD_CONTENT = `# Label Print Service for Brother QL-570

A local HTTP service that prints labels directly to a Brother QL-570 via USB, bypassing macOS driver requirements.

## Quick Start

1. Extract this folder to your Mac
2. Open Terminal and navigate to the folder:
   \`\`\`bash
   cd ~/Downloads/label-print-service
   \`\`\`
...
| Label    | Dimensions   | Brother Part |
|----------|--------------|--------------|
| 62x100   | 62mm × 100mm | DK-11202     |
| 62       | 62mm square  | DK-22205     |
| 29x90    | 29mm × 90mm  | DK-11201     |
| 17x54    | 17mm × 54mm  | DK-11204     |

## Troubleshooting

- **"Device not found"**: Ensure printer is connected and powered on
- **"No backend available"**: Run \`brew install libusb\`
- **Check printer connection**: \`brother_ql discover\`
`;

/**
 * Downloads the print service files as a ZIP archive
 */
export async function downloadPrintServiceZip(): Promise<void> {
  const zip = new JSZip();
  const folder = zip.folder('label-print-service');
  ...
  folder.file('main.py', MAIN_PY_CONTENT);
  folder.file('requirements.txt', REQUIREMENTS_TXT_CONTENT);
  folder.file('README.md', README_MD_CONTENT);
  ...
  link.href = url;
  link.download = 'label-print-service.zip';
