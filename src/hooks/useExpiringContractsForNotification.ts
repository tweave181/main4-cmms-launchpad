  ...
  const { data: expiringContracts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const contractsInReminderWindow = contracts.filter(contract => {
        if (!contract.reminder_days_before || !Array.isArray(contract.reminder_days_before) || contract.reminder_days_before.length === 0) return false;
        
        const endDate = new Date(contract.end_date);
        const reminderDays = Math.min(...contract.reminder_days_before);
